...
def load_and_clean_data(filepath):
    '''
    Read the df from file specified in NYC_Hospital_Analysis.ipynb
    '''
...
def filter_boroughs(df):
    ...
    df['Zip Code - 3 digits'] = df['Zip Code - 3 digits'].astype(object).astype(str)
    df['Zip3'] = df['Zip Code - 3 digits'].str.strip().str[:3]
...
def prepare_features(df):
    ...
    num_cols = ['Length of Stay', 'APR DRG Code', 'APR MDC Code', 'APR Severity of Illness Code']

    for col in num_cols:
        df[col] = pd.to_numeric(df[col].astype(object), errors='coerce')

    # Convert currency columns
    currency_cols = ['Total Charges', 'Total Costs']
    for col in currency_cols:
        df[col] = pd.to_numeric(
            df[col].astype(object).str.replace('$', '', regex=False).str.replace(',', '', regex=False),
            errors='coerce'
        )
